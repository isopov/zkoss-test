...
public class OrderController extends SelectorComposer<Component> {
    ...
    private final ListModel<OrderItem> orderItemsModel = new ListModelList<OrderItem>();
    ...
    public OrderController() {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            ((ListModelList<OrderItem>) orderItemsModel)
                    .add(new OrderItem("XYZ-00" + i, 2, car.getCost() + 3000.0, car));
        }
    }
    ...
    @Listen("onCreate = #win")
    public void init() {
        Clients.showNotification("Accept terms of service before submit the order", "info", tosCheckbox, "end_center",
                3000);
    }
